...
def validate_snapshot_name(name):
    if not name or len(name) < 7:
        raise argparse.ArgumentTypeError("Snapshot name must be at least 7 characters long.")
    return name

parser = argparse.ArgumentParser(description="Create a VM snapshot and generate an Excel report.")
parser.add_argument("--ip", required=True, help="Comma-separated list of IP addresses.")
parser.add_argument("--name", required=True, type=validate_snapshot_name, help="Snapshot name (min 7 characters).")
...
ip_list = list(set(ip.strip() for ip in args.ip.split(",")))
found_vms = []


for ip in ip_list:
    ip = ip.strip()
    vm = find_vm_by_ip(vms, ip)
    if vm:
        print(f"Found VM: {vm.name} (IP: {ip})")
        found_vms.append((vm, ip))
    else:
...
for vm, ip in found_vms:
    if not create_snapshot(vm, snapshot_name, full_description, memory_flag, args.quiesce):
        print(f"Snapshot creation failed for {vm.name} (IP: {ip}), continuing with other VMs.")
        exit(1)
